...
# 차트
 - kubernetes 리소스 yaml 파일을 템플릿으로 만들고 메타정보파일 등을 압축한 파일
 - Chart.yaml 파일은 차트의 필수 요소
 - chart 이름은 소문자와 숫자여야 한다. 단어는 대시(-)로 구분할 수 있다.
### 파일 구조
~~~
wordpress/
...
// charts/, crds/, templates/ 디렉터리와 나열된 파일명의 사용을 예약한다. 다른 파일들은 그대로 남는다.
~~~

### chart.yaml

``` yaml
apiVersion: 차트 API 버전 (필수) 최소 헬름 3을 필요로 하는 헬름 차트에 대해 v2 여야 한다
name: 차트명 (필수)
...
### values.yaml
 - 템플릿을 고려할때, 필수적인 값을 제공하는 values.yaml 은 다음과 같다.
``` yaml
